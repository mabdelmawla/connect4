...
static _t_str_c4_hal_graphics_window gstr_main_win_cfg = {
		.height =C4_GAME_DRAW_WINDOW_HIGHT,
		.width = C4_GAME_DRAW_WINDOW_WIDTH,
		.bg_color = {0,0,0,0},
		.pt_window_start = {0,0},
		.win_name = "Connect 4"
};
...
static void initialize_objects(void){
	...
	objs[cnt].uni_obj.rectangle.top_right.x = C4_GAME_DRAW_WINDOW_WIDTH - C4_GAME_DRAW_UNIT;
	objs[cnt].uni_obj.rectangle.top_right.y = C4_GAME_DRAW_WINDOW_HIGHT - C4_GAME_DRAW_UNIT;
	cnt++;

	//draw coin pointer
	objs[cnt].type = C4_HAL_GRAPHICS_CIRCLE;
	objs[cnt].color.r = 1;
	objs[cnt].uni_obj.circle.center.x = C4_GAME_DRAW_UNIT * 3 / 2;
	objs[cnt].uni_obj.circle.center.y = C4_GAME_DRAW_WINDOW_HIGHT - C4_GAME_DRAW_UNIT / 2;
	objs[cnt].uni_obj.circle.radius = C4_CIRCLE_R;
	objs[cnt].uni_obj.circle.finess = 120;
	cnt++;
	...
	for(int i = 0;i < 7; i++){
		...
		for(int j = 0; j < 6; j++){
			...
			g_slot_array[i][j].uni_obj.circle.center.x = C4_GAME_DRAW_UNIT * (3+ 2 * i) / 2;
			g_slot_array[i][j].uni_obj.circle.center.y = C4_GAME_DRAW_UNIT * (3+ 2 * j)/ 2;
