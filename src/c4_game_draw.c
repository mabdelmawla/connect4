...
static _t_str_c4_hal_graphics_window gstr_main_win_cfg = { .height =
		C4_GAME_DRAW_WINDOW_HIGHT, .width = C4_GAME_DRAW_WINDOW_WIDTH,
		.bg_color = { 0, 0, 0, 0 }, .pt_window_start = { 0, 0 }, .win_name =
				"Connect 4" };
...
static void initialize_objects(void) {
	...
	for (int i = 0; i < 7; i++) {
		...
		objs[cnt].uni_obj.circle.center.x = C4_GAME_DRAW_UNIT * (3 + 2 * i) / 2;
		objs[cnt].uni_obj.circle.center.y = C4_GAME_DRAW_WINDOW_HIGHT
				- C4_GAME_DRAW_UNIT / 2;
...
_t_c4_err c4_game_draw_init(_t_c4_hal_graphics_key_callback cb,
